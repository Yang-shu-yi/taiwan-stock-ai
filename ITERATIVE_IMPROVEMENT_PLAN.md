...
4. 只採用可驗證改善：主要 KPI 為扣成本後超額期望報酬；勝率只作次要指標。
...
| I-011 | 判讀語意 | 綜合低分／紅燈容易被誤讀成公司品質差 | 優質回檔與基本面惡化無法區分 | P0 | 保留綜合分數與燈號，另列品質、估值、時機、事件風險 |
| I-012 | 通知品質 | LINE 基本面文字過於籠統 | 使用者不知道要等什麼條件、何時取消觀察 | P0 | 顯示實際營收數字、估值依據與具體確認條件 |

## 2026-08-10 第一輪四維判讀

- 保留既有綜合分數與綠／黃／紅燈，不改動正式訊號門檻。
- 新增公司品質、估值吸引力、進場時機、事件風險；缺值顯示 `N/A` 或「待確認」，不補成中性分數。
- 公司品質第一輪以官方月營收趨勢作為基線，尚不宣稱等同完整財報品質。
- 估值使用 TWSE／TPEx 官方本益比、股價淨值比與殖利率，優先採同業相對排名，資料不足時退回同市場。
- 新聞只做公司事件風險初篩，不把新聞篇數直接加進綜合分數。
- LINE 改為「分數＋事件風險＋白話基本面＋具體等待／取消條件」。
- 驗證方式：先確認資料覆蓋率與訊息可讀性，再以獨立訊號日做消融測試，不因本輪新增欄位直接調高選股權重。
...
現行版本採版本化訊號、下一可交易開盤進場、手續費/證交稅/滑價與非重疊等權資金曲線；完整定義見 `PERFORMANCE_SYSTEM_V2.md`。
...
- [x] 新增 `signal_id`、run type、execution environment 與 strategy/feature version。
- [x] 新增策略與成本參數快照，正式/研究資料隔離。
- [x] 中小雷達改為 shadow-only 分帳。
- [x] 建立固定 logistic baseline、時序 walk-forward 與 OOF 機率校準。
- [x] 加入集中度、換手、流動性與真實資金曲線回撤。
