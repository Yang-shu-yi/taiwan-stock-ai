...
5. 監控清單個股必須「每一檔」都產出決策卡，不可省略任何一檔
6. 個股決策卡的價格與漲跌幅必須使用【監控股即時報價】中的數據，嚴禁編造
7. 每張決策卡的正向/負向依據必須引用具體新聞事件，禁止使用「公司基本面穩固」「產品線豐富」等通用廢話
8. 若某檔個股無直接相關新聞，請寫「無直接相關新聞，僅依報價與大盤連動分析」
...
3. 僅能引用提供資料與新聞，不可編造數據
4. 內容精簡、分段清楚、每段 1-3 行為主
5. 監控清單個股必須「每一檔」都產出回顧卡，不可省略任何一檔
6. 個股回顧卡的價格與漲跌幅必須使用【監控股即時報價】中的數據，嚴禁編造
7. 今日贏家/輸家必須根據【監控股即時報價】的漲跌幅排序填寫
8. 每張回顧卡的正向/負向重點必須引用具體新聞事件，禁止使用「公司基本面穩固」「產業前景看好」等通用廢話
9. 若某檔個股無直接相關新聞，請寫「無直接相關新聞，僅依報價與大盤連動分析」
...
def generate_report(
    ...
    report = _call_llm(
        system, user, groq_key, gemini_key, temperature=0.5, max_tokens=5000
    )
